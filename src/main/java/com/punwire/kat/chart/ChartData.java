...
public class ChartData {
    ...
    public void calculateRange(ChartFrame chartFrame) {
...
//            chartFrame.updateHorizontalScrollBar();
    ...
    {
        try {
            allData = mongoDb.getDataSet(stock.getSymbol(),interval.getLengthInSeconds()/60);
            size = allData.getItemsCount();
    ...
    public void setVisible(Dataset visible) {
    ...
    public double getMoveLoc(double value, Rectangle rect, Point point) {
        double r = rect.getMinX();
        int p = getPeriod();
        double r1 = ((value + 0.5D) / (double) getPeriod());
