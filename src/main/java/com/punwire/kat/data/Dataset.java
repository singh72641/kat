...
public class Dataset implements Serializable {
    ...
    public void setOpenIntAt(int index, double value) {
        if (index < 0 || index > data.size()) {
            return;
        }
        data.get(index).setOpenInterest(value);
    }

    public double getOpenIntAt(int index) {
        if (index < 0 || index > data.size()) {
            return 0;
        }
        return data.get(index) != null ? data.get(index).getOpenInterest() : 0;
    }
    ...
    public double getLastOpenInt() {
        return data.get(getLastIndex()) != null ? data.get(getLastIndex()).getOpenInterest() : 0;
    }
    ...
    public double getMin(int price) {
        ...
        switch (price) {
            ...
            case OPENINT_PRICE:
                for (DataItem item : list) {
                    if (item != null && value > item.getOpenInterest())
                        value = item.getOpenInterest();
                }
                return value;
        }
        return 0;
    ...
    public double getMinNotZero(int price) {
        ...
        switch (price) {
            ...
            case OPENINT_PRICE:
                for (DataItem item : list) {
                    if (item != null && item.getOpenInterest() != 0 && value > item.getOpenInterest())
                        value = item.getOpenInterest();
                }
                return value;
        }
    ...
    public List<DataItem> getVisibleItems(int period, int end) {
        System.out.println("GetVisisbleItems " + period + " " + end);
    ...
    public static Dataset LOG(Dataset d) {
        ...
        for (int i = 0; i < count; i++) {
            if (d.getDataItem(i) != null) {
                ...
                double openInt = Math.log10(d.getOpenIntAt(i));

                result.setDataItem(i, new DataItem(d.getTimeAt(i), open, high, low, close, volume, openInt));
            }
        }
    ...
    public static Dataset SUM(Dataset d1, Dataset d2) {
        ...
        for (int i = 0; i < count; i++) {
            result.setDataItem(i, new DataItem(
                    ...
                    d1.getOpenIntAt(i) + d2.getOpenIntAt(i)));
    ...
    public static Dataset SMA(Dataset dataset, int period) {
        ...
        for (int i = j + period - 1; i < count; i++) {
            ...
            for (int k = 0; k < period; k++) {
                ...
                openInt += dataset.getOpenIntAt(i - k);
            }
            ...
            openInt /= (double) period;

            result.setDataItem(i, new DataItem(time, open, high, low, close, volume,openInt));
        }
        return result;
    }
    ...
    public static Dataset EMA(Dataset dataset, int period) {
        ...
        for (int i = j; i < period + j && i < count; i++) {
            ...
            openInt += dataset.getOpenIntAt(i);
            ...
            if (i == period + j - 1) {
                ...
                result.setDataItem(i, new DataItem(dataset.getTimeAt(i), open, high, low, close, volume,openInt));
            } else {
        ...
        for (int i = period + j; i < dataset.getItemsCount(); i++) {
            ...
            openInt = (dataset.getOpenIntAt(i) - openInt) * k + openInt;
    ...
    public static Dataset WMA(Dataset dataset, int period) {
        ...
        for (int i = period + j; i < count; i++) {
            ...
            for (int k = i - period; k < i; k++) {
                ...
                openInt += (period - i + k + 1) * dataset.getOpenIntAt(k);
    ...
    public static Dataset TEMA(Dataset dataset, int period) {
        ...
        for (int i = j; i < count; i++) {
            ...
            if (ema1.getCloseAt(i) != 0 && ema2.getCloseAt(i) != 0 && ema3.getCloseAt(i) != 0) {
                ...
                openInt = 3 * ema1.getOpenIntAt(i) - 3 * ema2.getOpenIntAt(i) + ema3.getOpenIntAt(i);
            }
    ...
    public static final int OPENINT_PRICE = 5;
    public static final String OPEN = "Open";
    public static final String HIGH = "High";
    public static final String LOW = "Low";
    public static final String CLOSE = "Close";
    public static final String VOLUME = "Volume";
    public static final String OPEN_INT = "OpenInt";
